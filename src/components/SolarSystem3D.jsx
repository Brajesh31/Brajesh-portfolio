...
const PLANETS = [
  { id: "about",        label: "About",          distance: 25,  speed: 0.002, colors: { core: "#8B4513", surface: "#CD853F", glow: "#DEB887" }, startAngle: 0   },
  { id: "experience",   label: "Experience",     distance: 35,  speed: 0.002, colors: { core: "#8A2BE2", surface: "#9370DB", glow: "#DDA0DD" }, startAngle: 36  },
  { id: "ai-skills",    label: "AI Skills",      distance: 45,  speed: 0.002, colors: { core: "#4169E1", surface: "#6495ED", glow: "#87CEEB" }, startAngle: 72  },
  { id: "web-skills",   label: "Web Dev",        distance: 55,  speed: 0.002, colors: { core: "#228B22", surface: "#32CD32", glow: "#90EE90" }, startAngle: 108 },
  { id: "blog",         label: "Blog",           distance: 65,  speed: 0.002, colors: { core: "#FF8C00", surface: "#FFA500", glow: "#FFD700" }, startAngle: 144 },
  { id: "projects",     label: "Projects",       distance: 75,  speed: 0.002, colors: { core: "#20B2AA", surface: "#48D1CC", glow: "#AFEEEE" }, startAngle: 180 },
  { id: "services",     label: "Services",       distance: 85,  speed: 0.002, colors: { core: "#2F4F4F", surface: "#708090", glow: "#B0C4DE" }, startAngle: 216 },
  { id: "community",    label: "Community",      distance: 95,  speed: 0.002, colors: { core: "#FF7F50", surface: "#FF6347", glow: "#FFA07A" }, startAngle: 252 },
  { id: "events",       label: "Talks & Events", distance: 105, speed: 0.002, colors: { core: "#B22222", surface: "#DC143C", glow: "#F08080" }, startAngle: 288 },
  { id: "contact",      label: "Contact",        distance: 115, speed: 0.002, colors: { core: "#800080", surface: "#9932CC", glow: "#DA70D6" }, startAngle: 324 }
];
          ...
          {brightMode && (
              <>
                ...
                <mesh scale={[3.5, 3.5, 3.5]}>
                  ...
                  <meshBasicMaterial color="#FFFACD" transparent opacity={0.05} side={2} />
                </mesh>
              </>
          )}
        </mesh>
  ...
  return (
      <group ref={orbitRef}>
        ...
        >
          ...
          {["projects", "services", "community", "events"].includes(planet.id) && (
              <>
                <mesh rotation={[Math.PI / 2 + 0.2, 0, 0]}>
                  <ringGeometry args={[baseSize * 1.5, baseSize * 2.2, 64]} />
                  <meshBasicMaterial color={colors.surface} transparent opacity={0.4} side={2} />
                </mesh>
          ...
          <Html distanceFactor={2} position={[0, -(baseSize + 3), 0]}>
            <div className={`text-lg font-bold pointer-events-none whitespace-nowrap shadow-md border px-4 py-2 rounded-xl transition-all duration-300 ${
                brightMode
            ...
            <span className={`font-extrabold ${hovered ? "animate-pulse" : ""}`} style={{ color: colors.surface }}>
              {planet.label}
            </span>
            </div>
          </Html>
...
function SolarSystemScene({ onPlanetClick, onHover }) {
  ...
  return (
      <group rotation={[-0.3, 0, 0]}>
        ...
        <pointLight ref={sunLightRef} position={[0, 0, 0]} intensity={3} color="#f5c36b" castShadow shadow-mapSize-width={2048} shadow-mapSize-height={2048} />
        <pointLight position={[40, 40, 40]} intensity={brightMode ? 0.3 : 0.8} color="#ffffff" />
        <pointLight position={[-40, 30, 60]} intensity={brightMode ? 0.2 : 0.6} color="#4a90e2" />
        <pointLight position={[0, -30, 30]} intensity={brightMode ? 0.1 : 0.4} color="#8a2be2" />
        {PLANETS.map((planet) => {
          ...
          return (
              <mesh key={`orbit-${planet.id}`} rotation={[Math.PI / 2, 0, 0]}>
                ...
                <meshBasicMaterial color={planet.colors.surface} transparent opacity={opacity} side={2} />
              </mesh>
          );
        })}
        <OrbitRings />
        ...
        {PLANETS.map((planet) => (
            <Planet key={planet.id} planet={planet} onClick={onPlanetClick} onHover={onHover} brightMode={brightMode} />
        ))}
        <Stars />
      </group>
  );
