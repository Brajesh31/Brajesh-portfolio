...
export default function LiveGitHubActivity({ username = "brajesh31" }) {
  ...
  useEffect(() => {
    const fetchGitHubActivity = async () => {
      try {
        // Increase per_page to 21 to fetch more events
        const response = await fetch(`https://api.github.com/users/${username}/events?per_page=21`);
        const data = await response.json();

        // Map and slice last 21 events
        const processedActivity = data.slice(0, 21).map(event => ({
          id: event.id,
          type: event.type,
        ...
        setActivity([
          // You may want to increase mock items too if needed
